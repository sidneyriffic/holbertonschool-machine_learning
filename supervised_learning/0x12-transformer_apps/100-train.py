...
Dataset = __import__('100-dataset').Dataset
create_masks = __import__('100-create_masks').create_masks
Transformer = __import__('100-transformer').Transformer
...
def train_transformer(N, dm, h, hidden, max_len, batch_size, epochs):
    ...
    print("Execute model eagerly?", tf.executing_eagerly())
    ...
    #    print(i)
    input = tf.keras.Input((None, None))
    out = tf.keras.transformer(input, )
    for epoch in range(epochs):
        batch = 0
        print(dataset.data_train)
        #data_iter = tf.make_one_shot_iterator(dataset.data_train)
        #def train_step(input, target):
        for (batch, (input, target)) in enumerate(dataset.data_train):
